...
export class SaleitemsComponent implements OnInit {
  ...
  addProduct(): void {
    ...
    `,
      ...
      preConfirm: () => {
        const product_id = +(
          document.getElementById('product_id') as HTMLInputElement
        ).value;
        const discount_percent = +(
          document.getElementById('discount_percent') as HTMLInputElement
        ).value;
        const start_date = (
          document.getElementById('start_date') as HTMLInputElement
        ).value;
        const end_date = (
          document.getElementById('end_date') as HTMLInputElement
        ).value;

        if (!product_id || product_id <= 0)
          Swal.showValidationMessage('Product ID must be positive');
        if (!discount_percent || discount_percent < 0 || discount_percent > 100)
          Swal.showValidationMessage('Discount percent must be 0-100');
        if (!start_date || !end_date)
          Swal.showValidationMessage('Please enter valid dates');
  ...
  updateItem(productId: string | number | null) {
    Swal.fire({
    ...
    `,
      ...
      if (result.isConfirmed && result.value) {
        this._SaleitemsService
          .updateSaleItem(productId, result.value)
          .subscribe({
            next: (res) => {
              Swal.fire('Success!', 'Product added successfully.', 'success'),
                this.AllItems.set(res.data);
